const storyData = {
    ...
    's1_sketch_result': { speaker: "System", text: "The process is meditative. Eye to stone, pencil to paper. Half an hour disappears.\n\nWhen you finish and compare the sketch to the tablet, four glyphs don't match what you just drew. A pattern emerges: the glyphs that shifted are all determinatives for motion. As if the stone is describing something that won't hold still.\n\nIn the lower register, barely visible without the loupe, are small carved figures. They appear to be depicting people — but the proportions are wrong. Torso too long. Limbs too thin. And the hands: you count the joints on one sketched finger. Seven. You count again. Seven. You look at your own hand. You close the journal.", choices: [{ text: "Set pen down.", onSelect: () => { gameState.flags.Non_Human_Architect_Suspected = true; addJournalNote('Seven-Jointed Architecture', 'Carved figures on the Codex tablet have seven joints per finger. Not a sculptor\'s error — every other detail is flawless. The proportions are accurate to something.'); startDialogue('scene1_sound'); } }] },
    ...
    // --- SCENE 2: LOGISTICS ---
    'scene2_trench': { speaker: "Tariq", text: "He holds up his lantern so you can see.\n\nThere is a long crack running across the path — a trench, maybe a meter wide and very deep, like the desert floor split open. On the near side: your workers with their heavy supply carts. On the far side: two other carts that already crossed before the ground changed.\n\nThe dirt bridge across the middle is still there, but it looks soft. Sunken. Like something hollow is underneath it.\n\nTariq speaks quietly.\n\n'The men will not push a loaded cart across that. If the ground gives, the cart falls, maybe the man too. No one knows how deep it is.'\n\nHe pauses. 'There is a longer path that goes all the way around — safe, solid ground, but it adds thirty minutes and the carts have to go single file. Or someone could walk across first to show the men the ground still holds.'\n\n'The men will follow your example, Doctor. If you go first, they will go. That is how it works here.'", choices: [
        ...
        { text: "Reroute around the trench. Take the long way.", onSelect: () => { gameState.repLocal += 3; gameState.flags.eastTrenchResolved = true; gameState.flags.East_Trench_Avoided = true; }, nextScene: 'trench_reroute_result' },
    ...
    'trench_cross_result': { speaker: "System", text: "You step onto the dirt bridge. It holds — but it feels wrong under your feet. Not like solid desert. Like dry sand packed over empty air. With every step there is a faint vibration rising up through your boots, like something far below is humming.\n\nHalfway across you stop. The humming is stronger here. You look down at the crack on either side. Very dark. No sound from the bottom.\n\nYou keep walking. You make it across.\n\nBehind you, the workers watch in silence. Then one of them picks up his cart handles. Then another. Slowly, one by one, they follow.", choices: [{ text: "Examine the trench properly.", onSelect: () => startDialogue('ch1_m1_entry') }, { text: "Keep moving.", onSelect: () => closeDialogue() }] },
    'trench_pay_result': { speaker: "System", text: "You hold up the 500 EGP note. A young worker — maybe twenty years old — looks at it for a moment, then takes it without meeting your eyes.\n\nHe walks across. The bridge holds. He makes it.\n\nThe other workers watch him, then start moving.\n\nTARIQ (very quietly): 'He will remember that you paid him to take the risk instead of taking it yourself.'", choices: [{ text: "Keep moving.", onSelect: () => { updateHUD(); closeDialogue(); } }] },
    'trench_reroute_result': { speaker: "System", text: "Tariq nods and relays the order. You watch the workers' shoulders drop — the tension going out of them all at once.\n\nThey take the long way around without complaint. Thirty extra minutes, single file, through the dark. Nobody falls. Nobody gets hurt.\n\nWhen the last cart clears the detour and the men regroup, one of the older workers catches your eye and gives you a small nod. It means something.", choices: [{ text: "Examine the trench now that the path is clear.", onSelect: () => startDialogue('ch1_m1_entry') }, { text: "Keep moving.", onSelect: () => closeDialogue() }] },
    ...
    'flavor_boulder': {
        speaker: "System",
        text: "A boulder, half the size of a truck, worn smooth by centuries of wind. On the lee side, someone has carved a small mark — more recent than the stone's erosion. A plus sign inside a circle. Sam's field notation for 'worth coming back to.'\n\nAt the base of the boulder, wedged under the edge where only someone who knew to look would find it: a key on a plain ring, with a strip of tape labelled 'T.S.' in Sam's handwriting.",
        choices: [
            { text: "Take the key.", onSelect: () => { gameState.flags.sam_waypoint_found = true; if (!gameState.inventory.includes('Tool Shed Key')) { gameState.inventory.push('Tool Shed Key'); sndPickup && sndPickup(); } gameState.knowledgeCodex += 1; updateHUD(); closeDialogue(); } },
            { text: "Leave it. Move on.", onSelect: () => closeDialogue() }
        ]
    },
    'flavor_tool_shed_locked': {
        speaker: "System",
        text: "Padlocked. The outer lock is standard site issue — not the one you need. The inner lock's key was Sam's. Somewhere on this site he would have left it where someone he trusted could find it.\n\nLook around. He always left marks.",
        choices: [{ text: "Step back.", onSelect: () => closeDialogue() }]
    },
    'flavor_cactus': {
        speaker: "System",
    ...
    'flavor_spoil_mound': {
        ...
        choices: [
            { text: "Dig out the shard.", onSelect: () => { gameState.knowledgeCodex += 1; if (!gameState.inventory.includes('Glass Shard')) gameState.inventory.push('Glass Shard'); closeDialogue(); }, nextScene: 'shard_found_result' },
            { text: "Let it be.", onSelect: () => closeDialogue() }
        ]
    },
    'shard_found_result': {
        speaker: "System",
        text: "You work it free with your fingers. A triangular fragment of amber-coloured glass, about the size of your palm — but the colour is wrong for glass. Too deep. And the edges aren't fractured. They're ground.\n\nThis was made, not broken.\n\nYou add it to your kit. The spoil mound didn't produce it by accident. Someone buried this deliberately.",
        choices: [{ text: "Pocket it carefully.", onSelect: () => { sndPickup && sndPickup(); updateHUD(); closeDialogue(); } }]
    },
    ...
    'flavor_tool_shed': {
        ...
        choices: [
            { text: "Try the padlock.", onSelect: () => { if (gameState.inventory.includes('Tool Shed Key')) startDialogue('flavor_tool_shed_opened'); else startDialogue('flavor_tool_shed_locked'); }},
    ...
    'ch1_call_ayo_hour_before': { speaker: "Ayo", text: "'He called me an hour before the collapse. Said he was worried about the substrate reading. Said you had pushed back and he thought you were right to. Said the numbers didn't match the previous week's.\n\n'I told him to trust his instincts. I told him if it felt wrong he should call off the shift.\n\n'He didn't. He was about to. He was writing the email. Then the collapse happened.'\n\nShe says this flat. She has been carrying this for fourteen months. She is giving it to you now because she thinks you are strong enough to carry a piece of it.\n\n'The report was wrong, Ellis. He knew. I've been waiting to tell you until you were sober enough to hear it. I think you are sober now.'", choices: [
        { text: "Close your eyes.", onSelect: () => { decreaseSanity(1.0); gameState.knowledgeHermetic += 3; gameState.flags.Saqqara_Was_Arranged = true; gameState.flags.memorializedPartner = true; gameState.flags.flashback_sam_done = true; closeDialogue(); } }
    ]},
    ...
    'ch1_night_watch': { speaker: "System", text: "The far edge of the site. Past the floodlights. The sand here is cold enough to see your breath.\n\nThe sky is so clear that the horizon looks fake — too clean a line between the desert and the stars. Orion is high. The Pleiades are where they should be. The moon is a fingernail.\n\nFor a second, standing here, the whole thing feels manageable. Just a job. Just a site.\n\nThen you notice the sand at your feet has a pattern in it. Not wind-made. Deliberate. Three concentric circles. Something walked out here earlier tonight and drew them.", choices: [
        { text: "Crouch and examine the pattern.", onSelect: () => { decreaseSanity(0.7); gameState.knowledgeAtlantean += 2; gameState.flags.night_watch_done = true; startDialogue('ch1_night_watch_examine'); } },
    ...
    'int_foreman_sam': { speaker: "System", text: "A manila folder with SAM OKAFOR written in black marker. Inside: printouts of Sam's previous field reports. Saqqara. The 2021 Sudan dig. The Istanbul conference paper on pre-dynastic substrate mapping.\n\nSomebody on this crew pulled Sam's file before the dig started.\n\nSomebody was researching Sam.", choices: [
        { text: "Take the folder.", onSelect: () => { if (!gameState.inventory.includes('Sam_File')) { decreaseSanity(0.8); gameState.knowledgeHermetic += 3; gameState.inventory.push('Sam_File'); updateHUD(); } closeDialogue(); } },
        { text: "Read the Saqqara report more carefully.", onSelect: () => startDialogue('int_foreman_sam_saqqara') }
    ]},
    'int_foreman_sam_saqqara': { speaker: "System", text: "The Saqqara report is Sam's final published fieldwork. Official cause of death: accidental collapse, substrate failure, unstable substrata.\n\nBut clipped to the back of the printout is a handwritten page. Sam's handwriting. A draft of the original substrate assessment he submitted — and then a second draft, the one filed, with numbers that are materially different. Passable where the original said marginal. Within tolerance where the original said investigate further.\n\nSam's note in the margin of the second draft, circled three times: 'These are not my numbers.'", choices: [
        { text: "Sit with this for a moment.", onSelect: () => { decreaseSanity(1.0); gameState.knowledgeHermetic += 2; gameState.flags.Saqqara_Was_Arranged = true; if (!gameState.inventory.includes('Sam_File')) { gameState.inventory.push('Sam_File'); } updateHUD(); closeDialogue(); } }
    ]},
    ...
    'int_dorm_worker': { speaker: "Worker", text: "He is sitting on the edge of his cot, still fully dressed, boots on. He has not been sleeping.\n\n'Doctor. I am glad you came.'\n\nHis name is Osman. He has worked digs for fifteen years.\n\n'I want to tell you something and I want you to not react in a way that makes the other men lose their nerve. Can you do that?'\n\n'Yes.'\n\n'Three nights ago I heard my mother's voice from below the east trench. My mother has been dead for eleven years. She was singing the song she always sang when she cooked.\n\n'I did not go toward it. I want you to know that I did not go toward it. But some of the younger men have been going toward it. In their sleep.'", choices: [
        { text: "'Thank you for telling me.'", onSelect: () => { decreaseSanity(0.4); gameState.knowledgeAtlantean += 2; gameState.trustTariq += 1; closeDialogue(); } },
    ...
    'int_dorm_missing': { speaker: "Worker", text: "He looks at you a long time before answering.\n\n'Farid from the night crew. Three nights ago. We found him in the morning at the far edge of the site, asleep sitting up, facing east. He did not remember walking there.\n\n'He quit that day. He said the dig is over a mouth.'\n\nHe pauses.\n\n'He is not a superstitious man, Doctor. That is why I am telling you.'", choices: [
        { text: "Stay with him a moment.", onSelect: () => { decreaseSanity(0.5); gameState.knowledgeAtlantean += 2; closeDialogue(); } }
    ]},
    ...
    'int_hlb_files': { speaker: "System", text: "The filing cabinet is unlocked. Inside: folders arranged by year going back to 1987.\n\nEach folder contains the same structure — a location (Giza, Saqqara, Memphis, Dashur), a date, a name, and a three-word assessment. You flip through several:\n\n'Did not survive. Unsuitable.'\n'Left immediately. Unsuitable.'\n'Found nothing. Unsuitable.'\n'Saqqara 2022 — OKAFOR, SAMUEL. Assessment: Suitable. Status: Deceased.'\n\nBelow Sam's folder: 'Giza 2024 — VANCE, ELLIS. Assessment: Suitable. Status: Active.'", choices: [
        { text: "Put the folders back exactly as they were.", onSelect: () => { decreaseSanity(1.0); gameState.knowledgeHermetic += 4; gameState.flags.Saqqara_Was_Arranged = true; closeDialogue(); } }
    ]},
    ...
    'ch2_whispers_listen': { speaker: "System", text: "You stand still and let it in.\n\nFor perhaps a minute. Then five. You are not sure how long.\n\nWhen you move again, three things have changed: you know — intuitively, without knowing how you know — the specific sequence of glyphs that unlocks the city's northeast chamber. You know it the way you know your own name. You also have a nosebleed, and you have lost approximately forty minutes of time you cannot account for.\n\nThe amber channel near the exit is slightly brighter than when you entered.", choices: [
        { text: "Move on.", onSelect: () => { gameState.flags.trapWhispersHeard = true; gameState.flags.city_sequence_known = true; addJournalNote('Northeast Chamber Sequence', 'Transmitted by the whisper chamber — not learned, installed. I know it the way I know my name. Nosebleed. ~40 min unaccounted. Sequence: Eye, Owl, Hand, Reed, Eye, Cobra.'); closeDialogue(); } }
    ]},
    ...
    'ch3_vendor': { speaker: "Tea Vendor", text: "A street vendor stirring a pot of boiling water looks up at you.\n\n'Mint tea, effendi? Good for the heat. Good for the nerves.'", choices: [
        ...
        { text: "Buy mint tea (20 EGP) — restores nerves.", onSelect: () => { if (gameState.funds >= 20) { gameState.funds -= 20; increaseSanity(3.0); startDialogue('ch3_vendor_tea'); } else { startDialogue('ch3_vendor_broke'); } } },
        { text: "Buy karkadeh (150 EGP) — boosts stamina recovery.", onSelect: () => { if (gameState.funds >= 150) { gameState.funds -= 150; if (!gameState.inventory.includes('Karkadeh')) gameState.inventory.push('Karkadeh'); updateHUD(); startDialogue('ch3_vendor_karkadeh'); } else { startDialogue('ch3_vendor_broke'); } } },
        { text: "Buy energy dates (80 EGP) — restores 5 stamina.", onSelect: () => { if (gameState.funds >= 80) { gameState.funds -= 80; gameState.stamina = Math.min(gameState.maxStamina, gameState.stamina + 5); updateHUD(); startDialogue('ch3_vendor_dates'); } else { startDialogue('ch3_vendor_broke'); } } },
    ...
    'ch3_vendor_tea': { speaker: "System", text: "You drink a hot cup of mint tea. The familiarity of the hot ceramic and the sweet mint soothes your frazzled nerves.\n\n(Sanity restored).", choices: [{ text: "Thanks.", onSelect: () => { updateHUD(); closeDialogue(); } }] },
    'ch3_vendor_karkadeh': { speaker: "Tea Vendor", text: "'Karkadeh — dried hibiscus from upper Aswan. My grandmother used to say it gives back what the desert takes. Drink slow.'\n\nThe sour-sweet liquid is shockingly cold. You feel the effect almost immediately — something in the acidity, or something in the altitude, or something he isn't saying.\n\n(Stamina regenerates faster for the rest of the run.)", choices: [{ text: "Finish the cup.", onSelect: () => { updateHUD(); closeDialogue(); } }] },
    'ch3_vendor_dates': { speaker: "Tea Vendor", text: "A paper cone of pressed dates — the Medjool variety, still slightly warm from sitting in the sun all evening. You eat them standing up.\n\nCalories hitting an empty stomach is not glamorous but it is effective.\n\n(Stamina restored +5)", choices: [{ text: "Thank him.", onSelect: () => { updateHUD(); closeDialogue(); } }] },
    ...
    'hostile_ministry_caught': {
        ...
        text: "A guard in a dark uniform steps directly into your path. Young. Steady. He doesn't reach for anything — he just stops you with his presence.\n\n'Hey. Stop right there.'\n\nHe points at a line of yellow tape strung between two stakes in the sand, about ten meters behind you.\n\n'You crossed the perimeter. That side is Ministry land. You need to go back — now — or I make a call and this gets complicated for both of us.'\n\nHe's not angry. He's doing his job.",
        choices: [
    ...
    'ch1_m3_physical_smell': {
        speaker: "System",
        text: "Old incense. Specifically: kyphi — the ancient Egyptian temple blend. Cedar, myrrh, raisins, wine, juniper. You only know this because you spent three weeks at the Cairo Museum reading the New Kingdom temple records with Sam.\n\nKyphi was used for one specific ritual purpose: to facilitate communication between the living and the inhabitants of the Duat. The underworld.\n\nThe scent is not coming from the sand. It is coming from the air at approximately head height, concentrated in the space where the figure was standing. Like someone burning incense has been standing here for a long time and has recently left.\n\nYou look down at the sand one more time. Still no footprints.\n\nThen, on the top of the nearest fuel barrel: a small card. No name. No number. Just three concentric arcs embossed in silver foil. A logo you have never seen, but the kind of thing a company puts on property it wants to be able to identify later. Whoever was standing here had solid weight — they left this.\n\nSomething was standing here that does not weigh what a person weighs — or someone was standing here who knew exactly how not to leave a trace.",
        choices: [{
            text: "Pocket the card. Document everything.",
            onSelect: () => {
                gameState.knowledgeHermetic += 2;
                gameState.knowledgeAtlantean += 1;
                gameState.flags.Watcher_Confirmed_Real = true;
                if (!gameState.inventory.includes('Embossed Card')) gameState.inventory.push('Embossed Card');
                addJournalNote('The Concern — Embossed Card', 'Three concentric arcs in silver foil. Left on a fuel barrel by the figure — no footprints, kyphi scent, no weight signature. Someone is watching this dig and wanted us to find the card.');
                updateHUD();
    ...
    'ch1_all_missions_complete': {
        ...
        choices: [{
            ...
            onSelect: () => {
                gameState.flags.scene3Triggered = true;
                if (!gameState.inventory.includes('The Codex')) { gameState.inventory.push('The Codex'); sndPickup && sndPickup(); }
                updateHUD();
                closeDialogue();
